...
function binomialTest() {
	...
	if ( isNumberArray( arguments[ 0 ] ) ) {
		x = arguments[ 0 ];
		if ( x.length !== 2 ) {
			throw new Error( format( '1Gx9G,IF', x ) );
		}
		n = x[ 1 ] + x[ 0 ];
		x = x[ 0 ];
		if ( arguments[ 1 ] ) {
		...
		if ( !isNonNegativeInteger( x ) ) {
			throw new TypeError( format( '1Gx9H,IG', x ) );
		}
		if ( !isNonNegativeInteger( n ) ) {
			throw new TypeError( format( '1Gx2K,Ez', n ) );
		}
		if ( x > n ) {
			throw new TypeError( format( '1Gx9I,IH' ) );
		}
		if ( arguments[ 2 ] ) {
	...
	if ( alpha < 0.0 || alpha > 1.0 ) {
		throw new RangeError( format( '1Gx8V,Gb', 'alpha', alpha ) );
	}
	if ( opts.p === void 0 ) {
		p = 0.5;
	} else {
	...
	if ( p < 0.0 || p > 1.0 ) {
		throw new RangeError( format( '1Gx9J,II', 'p', p ) );
	}
	...
	case 'two-sided':
		...
		break;
	default:
		throw new Error( format( '1Gx2X,3g,4S,GD,Gg,Jm', 'alternative', [ 'two-sided', 'less', 'greater' ].join( '", "' ), alt ) );
	}
