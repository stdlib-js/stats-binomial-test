...
function binomialTest() {
	...
	if ( isNumberArray( arguments[ 0 ] ) ) {
		x = arguments[ 0 ];
		if ( x.length !== 2 ) {
			throw new Error( format( '1Gx9G', x ) );
		}
		n = x[ 1 ] + x[ 0 ];
		x = x[ 0 ];
		if ( arguments[ 1 ] ) {
		...
		if ( !isNonNegativeInteger( x ) ) {
			throw new TypeError( format( '1Gx9H', x ) );
		}
		if ( !isNonNegativeInteger( n ) ) {
			throw new TypeError( format( '1Gx2K', n ) );
		}
		if ( x > n ) {
			throw new TypeError( format( '1Gx9I', x, n ) );
		}
		if ( arguments[ 2 ] ) {
	...
	if ( alpha < 0.0 || alpha > 1.0 ) {
		throw new RangeError( format( '1Gx8V', 'alpha', alpha ) );
	}
	if ( opts.p === void 0 ) {
		p = 0.5;
	} else {
	...
	if ( p < 0.0 || p > 1.0 ) {
		throw new RangeError( format( '1Gx9J', 'p', p ) );
	}
	...
	case 'two-sided':
		...
		break;
	default:
		throw new Error( format( '1Gx4S', 'alternative', [ 'two-sided', 'less', 'greater' ].join( '", "' ), alt ) );
	}
